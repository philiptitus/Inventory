...
import { fetchCounties } from "@/lib/countyApi";
...
const CountiesSection: React.FC<CountiesSectionProps> = ({
  ...
  const fetchAndSetCounties = async (search?: string) => {
    setLoading(true);
    try {
      const token = typeof window !== 'undefined' ? (localStorage.getItem("token") || undefined) : undefined;
      if (!token) throw new Error("No token found");
      const data = await fetchCounties(1, 10, token, search ?? searchTerm);
  ...
  React.useEffect(() => {
    ...
    searchTimeout.current = setTimeout(() => {
      fetchAndSetCounties(searchTerm);
    }, 400);
    return () => {
      if (searchTimeout.current) clearTimeout(searchTimeout.current);
    };
        ...
        {loading && (
          <div className="flex justify-center items-center py-8">
            <div className="flex flex-col items-center">
              <div className="mb-2 text-red-200 text-sm">Loading counties...</div>
